...
#将同一列中用分隔符隔开的内容处理成哑变量
app_genres <- app %>% select(Rating, Genres) %>% 
  cSplit_e("Genres", ";", mode = "binary", type = "character", fill = 0, drop = T) #splitstackshape包
...
ts_s <- ts_s %>% mutate(mean_arpu = rowMeans(select(., starts_with("arpu")), na.rm = T),
                        ...
                        mean_zt = rowMeans(select(., starts_with("zt")), na.rm = T)) #特定列均值
...
poi_yx_edu <- subset(poi, adname == '越秀区' & yfLevel_1 == '教育') #poi越秀区教育
...
#数值变量分箱
app_c$Reviews_bin <- discretize(app_c$Reviews, method = "frequency", breaks = 10) #arule包
...
write.xlsx(na_origin[id100, ], "D:/K/云房/数据/数据自动化清理/分类测试100.xlsx") #随机抽取样本导出

set.seed(100)
random100 <- sample(unique(huhaomysql$hunitid), 100, replace = FALSE) #不放回抽样
...
#训练集和测试集划分
set.seed(1)
ind <- createDataPartition(model$TYPE1, p = 0.75, list = F) #根据因变量比例划分
train <- model[ind, ]
test <- model[-ind, ]

#长宽数据转换
k10 <- cbind.data.frame(result1, result2, result3, result4, result5, result6, result7, result8, result9, result10)
k10_long <- melt(k10, id.vars = "k", variable.name = "result", value.name = "value") #宽数据→长数据

x_wide <- reshape(x_long, timevar = 'Var3', idvar = c('Var1', 'Var2'), direction = 'wide') #长数据转宽数据
spread(key = sentiment, value = n, fill = 0) #长转宽，将sentiment的取值变为变量，n变为变量取值，用0填充缺失

tidy_books %>% 
  inner_join(get_sentiments("bing"), by = "word") %>% 
  count(word, sentiment, sort = TRUE) %>% 
  acast(word ~ sentiment, value.var = "n", fill = 0)  #长转宽
...
#数据合并
oly_dict <- merge(oly, dict, by.x = 'Country', by.y = 'Code', all = TRUE) #全连接
tidy_books %>% anti_join(stop_words, by = "word") #剔除特定内容
tidy_books %>% 
  filter(book == "Emma") %>% 
  inner_join(nrcjoy)  #内连接
...
chapters_gamma <- chapters_gamma %>% 
  separate(col = document, into = c("title", "chapter"), sep = "_", convert = T) #convert将字符转为数值
...
#因子顺序处理
value_2017$age_group <- factor(value_2017$age_group, levels = c("80后", "90后", "95后", "00后"))
